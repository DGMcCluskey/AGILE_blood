...
analyse_cluster <- function(cluster_name) {
  ...
  model <- tryCatch({
    lmer(
      delta ~ treatment * time_new + Age + Drift + (1 | donor),
      data = dat_clust
    )
  }, error = function(e) {
...
ggplot(t, aes(x = time_new, y = marker, fill = diff_capped))+geom_tile(colour = "black")+
    facet_wrap(~labels, ncol=11)+scale_fill_distiller(palette = "RdBu", limits = max(abs(t$diff_capped)) * c(-1, 1))
ggplot(myeloid, aes(x = time_new, y = marker, fill = diff_capped))+geom_tile(colour = "black")+
    facet_wrap(~labels, ncol=11)+scale_fill_distiller(palette = "RdBu",
    limits = max(abs(myeloid$diff_capped)) * c(-1, 1))
```
...
model_input <- exp_by_donor %>%
  filter(labels %in% myeloid) %>% #change cell type/group here
  filter(marker == moi)
