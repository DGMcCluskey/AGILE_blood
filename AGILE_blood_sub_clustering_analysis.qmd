...
plot_input <- filter(plot_input, !marker %in% c("CD45", "CD4", "CD8", "CD15", "CD66b", "CD3"))
...
```{r}
t_cells <- exp_by_donor %>% filter(labels %in% c("CD4+T", "Treg", "Naive CD8+T", "Memory CD8+T", "T")) %>%
    filter(marker %in% "KI67")
ggplot(t_cells, aes(x = time_new, y = median, colour = treatment, group = treatment))+geom_point()+theme_bw()+
