...
### the first plot here is just a sense check to see how things behave (in this case in tregs)
...
exp <- expression_input %>%
  group_by(donor, treatment, labels, marker, time_new) %>%
  summarise(mean = mean(expression, na.rm = TRUE), .groups = "drop") %>%
  group_by(donor, treatment, labels, marker) %>%
  mutate(
    baseline = if (any(time_new == "-72h")) {
      mean[time_new == "-72h"]
    } else {
      0  # Set baseline to 0 if no -72h value exists
    }
  ) %>%
  ungroup()

ggplot(filter(exp, labels %in% "Treg"), aes(x = time_new, y = mean, colour=treatment, group = treatment))+
    geom_smooth(se=F)+
    geom_point()+facet_wrap(~marker, scales = "free_y")+theme_bw()
```

## i'll inspect general changes in marker expression to try and hone in on specific marker/cluster pairs
```{r}
