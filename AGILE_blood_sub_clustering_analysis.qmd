...
### here i will also load in some further patient metadata (age and date) and add to df
### i made a column in excel called "Drift", which is baseically the time (days) since the first sample, to account for processing differences over the months
```{r}
metadata <- readxl::read_xlsx("Participant_info.xlsx")
metadata <- metadata[, c(1,2,4,6)]
names(metadata)[1] <- "donor"
names(metadata)[3] <- "date"
neut.data <- left_join(neut.data, metadata, by = "donor")
neut.data$Age <- as.numeric(neut.data$Age)
neut.data$date <- as.factor(neut.data$date)
neut.data$Drift = as.numeric(neut.data$Drift)
```

## save/load dataframe
```{r}
#saveRDS(neut.data, file = "AGILE_blood_dataframe.rds", compress = F)
neut.data <- readRDS("AGILE_blood_dataframe.rds")
```

## as a quality control/inspection, plot all markers in a histogram and scatter, to see if they possess positivity
```{r}
expression_input <- pivot_longer(neut.data, names_to = "marker", values_to="expression", 1:31)
p1 <- ggplot(filter(expression_input, labels %in% c("CD4+T", "Treg")), aes(x=expression))+theme_bw()+
    facet_wrap(~marker, scales = "free")+geom_density()+
    theme(strip.text=element_text(size=14, colour = "black", face = "bold"))
p1
ggsave(plot=p1, filename="qc_marker_expression_hisotgrams.png", dpi = 500,height=12,width=14)
```
...
sample_metadata <- neut.data %>%
  dplyr::select(sample_id, donor, treatment, time_new, Age, Drift) %>%
  distinct()
...
cluster_counts <- neut.data %>%
  group_by(sample_id, donor, labels, treatment, time_new, Age, Drift) %>%
  summarise(cluster_count = n(), .groups = "drop")

# Fill in missing combinations with 0s
cluster_counts <- all_combos %>%
  left_join(cluster_counts, by = c("sample_id", "donor", "labels", "treatment", "time_new", "Age", "Drift")) %>%
...
delta_data <- counts_summary %>%
  ...
  mutate(
    ...
    delta = log2((prop + 1e-6) / (baseline_prop + 1e-6))
  ) %>%
    ungroup() %>%
  filter(time_new != "-72h")
```
...
## add some rows that give baseline (-72h) some 0 values just for plotting
## plot the results and save
```{r}
emm_all_extended <- bind_rows(
  emm_all,
  expand.grid(
    treatment = unique(emm_all$treatment),
    cluster_id = unique(emm_all$cluster_id),
    time_new = "-72h"
  ) %>%
    mutate(emmean = 0, lower.CL = 0, upper.CL = 0)
) %>%
  mutate(
    time_new = factor(time_new, levels = c("-72h", "0h", "6h", "24h", "48h", "72h"))
  )

plot1 <- ggplot(emm_all_extended, aes(x = time_new, y = emmean, colour = treatment, group = treatment)) +
  ...
  labs(
    title = "Log2 fold change of cluster proportions over time",
    y = "Log2FC",
    x = "Timepoint",
    colour = "Treatment"
  ) +
...
## change in absolute counts
### instead of doing proportions, i will take the total number of cells (50uL of cell suspension was recorded for all samples, so we can compare them) and assess change
```{r}
neut.data$time_new <- factor(neut.data$time, levels = c("-72h", "0h", "4h", "6h", "24h", "48h", "72h"),
labels = c("-72h", "0h", "6h", "6h", "24h", "48h", "72h"))
# Step 1: Get valid donor × timepoint × treatment combos
valid_combos <- neut.data %>%
  distinct(donor, time_new, treatment)

# Step 2: Get all cluster labels
all_labels <- unique(neut.data$labels)

# Step 3: Create full valid combinations with all labels
complete_combos <- expand_grid(
  valid_combos,
  labels = all_labels
)

# Step 4: Count observed cells per group
observed_counts <- neut.data %>%
  count(labels, donor, time_new, treatment, name = "n")

# Step 5: Left join observed counts to full combo grid
counts <- complete_combos %>%
  left_join(observed_counts, by = c("labels", "donor", "time_new", "treatment")) %>%
  mutate(n = replace_na(n, 0))
clusters <- unique(counts$labels)
```

```{r}
analyse_cluster <- function(cluster_name) {
  dat_clust <- counts %>% filter(labels == cluster_name)
  
  model <- tryCatch({
    glmer(
      n ~ treatment * time_new + (1 | donor),
      data = dat_clust, family = poisson
    )
  }, error = function(e) {
    message(paste("Model failed for cluster", cluster_name))
    return(NULL)
...
max_prob <- emm_all %>%
  group_by(cluster_id) %>%
  summarise(max_y = max(asymp.UCL, na.rm = TRUE)) %>%
  mutate(label_y = max_y +0.05)
...
## plot the results and save
```{r}
plot1 <- ggplot(emm_all, aes(x = time_new, y = rate, colour = treatment, group = treatment))+
  geom_line(position = position_dodge(width=0.4)) +
  geom_point(position = position_dodge(width=0.4)) +
...
plot1
#change name of plot if neuts are excluded
ggsave(plot=plot1,filename="absolute_counts.png", dpi = 500,
height = 8, width = 12, bg="white")
```
...
### to do this, i average by donor and then will average across treatment (in two stages), which will match the averaging by donor i do for the lmm (averaging straight to treatment here would give different results)
```{r}
expression_input <- pivot_longer(neut.data, names_to = "marker", values_to="expression", 1:31)
...
# Now aggregate *again* to match the treatment-level summary, 
# but using donor-level medians instead of raw expression
exp_by_treatment_from_donors <- exp_by_donor %>%
  group_by(treatment, labels, marker, time_new) %>%
  summarise(median = median(median, na.rm = TRUE), .groups = "drop")  # second median
```
...
```{r}
baseline_df <- exp_by_treatment_from_donors %>%
  filter(time_new == "-72h") %>%
  dplyr::select(treatment, labels, marker, baseline = median)
plot_input <- exp_by_treatment_from_donors %>%
  left_join(baseline_df, by = c("treatment", "labels", "marker"))
plot_input <- plot_input %>%
  mutate(change_from_baseline = median - baseline)
...
plot1 <- ggplot(t, aes(x = time_new, y = marker, fill = change_from_baseline))+geom_tile(colour = "black")+theme_bw()+
    facet_grid(labels~treatment)+scale_fill_distiller(palette = "RdBu", limits = max(abs(t$change_from_baseline)) * c(-1, 1))+theme(axis.text=element_text(size=12,colour="black"))+theme(strip.text = element_text(size=12,colour="black"))
plot1
ggsave(plot=plot1,filename="T_cell_marker_change.png", dpi=500,height=14,width=6,bg="white")
plot2 <- ggplot(myeloid, aes(x = time_new, y = marker, fill = change_from_baseline))+geom_tile(colour = "black")+theme_bw()+
    facet_grid(labels~treatment)+scale_fill_distiller(palette = "RdBu",
    limits = max(abs(myeloid$change_from_baseline)) * c(-1, 1))+theme(axis.text=element_text(size=12,colour="black"))+
        theme(strip.text = element_text(size=12,colour="black"))
plot2
ggsave(plot=plot2,filename="myeloid_marker_change.png", dpi=500,height=10,width=6,bg="white")
```
...
## before the lmm i also remove -72h(baseline) from the time column, as this will be used as a covariate and not an outcome
```{r}
exp_by_donor <- expression_input %>%
  group_by(donor, treatment, labels, marker, time_new, Age, Drift) %>%
  summarise(median = median(expression, na.rm = TRUE), .groups = "drop")
moi <- "CD25"
...
model_input <- exp_by_donor %>%
  filter(labels %in% t) %>% #change cell type/group here
  filter(marker == moi)
baseline_df <- model_input %>%
  filter(time_new == "-72h") %>%
  dplyr::select(donor, treatment, labels, marker, Age, Drift, baseline = median)
model_input <- model_input %>%
  left_join(baseline_df, by = c("donor", "treatment", "labels", "marker", "Age", "Drift"))
...
median_iqr <- function(x) {
  ...
  data.frame(y = med, ymin = q1, ymax = q3)
}
ggplot(model_input, aes(x = time_new, y = change_from_baseline, colour = treatment, group = treatment))+theme_bw()+
    stat_summary(fun=median, geom = "point")+
    stat_summary(fun=median, geom="line")+
    stat_summary(fun.data = median_iqr, geom="errorbar", width = 0.2)+
    facet_wrap(~labels, scales="free_y")

model_input <- filter(model_input, !time_new %in% "-72h")
clusters <- unique(model_input$labels)
```
...
analyse_cluster <- function(cluster_name) {
  ...
  model <- tryCatch({
    lmer(
      change_from_baseline ~ treatment * time_new + (1 | donor),
      data = dat_clust
    )
  }, error = function(e) {
    message(paste("Model failed for cluster", cluster_name))
...
## plot the results and save
```{r}
plot1 <- ggplot(emm_all, aes(x = time_new, y = emmean, colour = treatment, group = treatment)) +geom_hline(yintercept=0, linetype="dashed", colour = "black")+
  geom_line(position = position_dodge(width=0.4)) +
  geom_point(position = position_dodge(width=0.4)) +
  geom_errorbar(aes(ymin = lower.CL, ymax = upper.CL), width = 0.2,
  position = position_dodge(width=0.4)) +
...
## based on Tregs appearing to have a lot of changes i will pull them out and assess multiple marker expression on them in one go for efficiency
## i will run all markers then before plotting i'll pull out ones of interest/relevance
## i've also set the code up to substitute in celltype of choice and save the plot with the name of the inputted celltype
```{r}
exp_by_donor <- expression_input %>%
  group_by(donor, treatment, labels, marker, time_new, Age, Drift) %>%
  summarise(median = median(expression, na.rm = TRUE), .groups = "drop")
celltype <- c("DC") #change celltype here
model_input <- exp_by_donor %>%
  filter(labels %in% celltype)
baseline_df <- model_input %>%
  filter(time_new == "-72h") %>%
  dplyr::select(donor, treatment, labels, marker, Age, Drift, baseline = median)
model_input <- model_input %>%
...
model_input <- model_input %>%
  mutate(change_from_baseline = median - baseline)
model_input <- filter(model_input, !time_new %in% "-72h")
markers <- unique(model_input$marker)
```
...
## plot the results and save
## add some rows with 0 values at -72h just to highlight the deviation from baseline
```{r}
emm_all_extended <- bind_rows(
  emm_all,
  expand.grid(
    treatment = unique(emm_all$treatment),
    marker = unique(emm_all$marker),
    time_new = "-72h"
  ) %>%
    mutate(emmean = 0, lower.CL = 0, upper.CL = 0)
) %>%
...
treg_markers <- c("FOXP3", "CD25", "CTLA4", "CD28", "CD127", "CD39", "CD47", "CD62L", "CCR6", "CD69", "CXCR4", "PD1", "TIGIT", "KI67", "GLUT1", "CD4")
myeloid_markers <- c("CD14", "CD16", "CD11b", "CD1c", "CD15", "CD39", "CD47", "CD62L", "CD69", "CD86", "CXCR4", "HLA-DR", "KI67", "GLUT1")

marker_list <- switch(celltype,
  "Treg" = treg_markers,
  "Classical_mono" = myeloid_markers,
  "Intermediate_mono" = myeloid_markers,
  "Non_classical_mono" = myeloid_markers,
  "DC" = myeloid_markers,
  stop("Unknown celltype specified")
)

plot1 <- ggplot(emm_all_extended %>% filter(marker %in% marker_list), aes(x = time_new, y = emmean, colour = treatment, group = treatment)) +
  ...
  position = position_dodge(width=0.4)) +
  geom_text(
    data = contrast_all %>% filter(sig_label != "") %>% filter(marker %in% marker_list),
    aes(x = time_new, y = label_y, label = sig_label),
    inherit.aes = FALSE,
    size = 7
  ) +
...
## even if a marker is changing longitudinally in a celltype, it may not be very highly expressed in the first place, so this needs to be checked
```{r}
marker_use <- c("CD69", "KI67")
expressions_data <- pivot_longer(neut.data, names_to="marker", values_to="expression", 1:31)
ggplot(expression_input %>% filter(time_new %in% c("-72h", "6h") & marker %in% marker_use), 
aes(x=expression, y = labels, fill = labels))+theme_bw()+
    facet_wrap(~marker, scales = "free_x")+geom_density_ridges()+
    theme(strip.text=element_text(size=14, colour = "black", face = "bold"))+
    scale_fill_manual(values=cluster_palette)+
    theme(axis.text=element_text(size=14, colour = "black"))+ylab(NULL)+
    theme(axis.title=element_text(size=16, colour = "black"))
```