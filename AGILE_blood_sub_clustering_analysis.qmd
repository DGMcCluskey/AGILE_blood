...
```{r}
logfc_df <- exp_by_treatment %>%
  pivot_wider(names_from = treatment, values_from = median) %>%
  mutate(log2FC = log2((corona+1e3 / guinness+1e3)))
plot_input <- filter(logfc_df, !marker %in% c("CD45", "CD4", "CD8", "CD15", "CD66b", "CD3"))

ggplot(plot_input, aes(x = time_new, y = marker, fill = log2FC))+geom_tile()+
    facet_wrap(~labels, ncol=11)+scale_fill_distiller(palette = "RdBu")
```

## an alternative is to plot relative change in expression from baseline
```{r}
plot_input$rel_change <- plot_input$median/plot_input$baseline
ggplot(plot_input, aes(x = time_new, y = median, colour = labels, group = labels))+geom_point()+theme_bw()+
    facet_wrap(~marker+treatment)
```