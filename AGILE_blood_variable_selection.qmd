...
for (tp in timepoints) {
  ...
  perf_res <- perf(plsda_res, validation = "Mfold", folds = 5, progressBar = FALSE, nrepeat = 10)
  ...
  # Extract top markers
  top_vars <- selectVar(plsda_res, comp = 1)$name
  top_markers_by_time[[tp]] <- top_vars

  # Plots
  p_embed <- plotIndiv(plsda_res, comp = c(1,2), group = Y, legend = TRUE,
                       title = paste("PLS-DA:", tp),
                       ellipse = TRUE, star = F, cex = 2.5, pch = 16,
                       col=c("mediumseagreen", "darkorchid2"), style="ggplot2")
  
  plot_obj <- plotLoadings(plsda_res, comp = 1,
...
combo1
ggsave(plot=combo1, filename = "pls-da_embeddings.png", dpi=500, height=5,width=18)
```
