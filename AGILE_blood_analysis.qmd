...
## save annotated object here
```{r}
#saveRDS(sce, file="agile_blood_annotated.rds", compress=F)
...
## annotated umaps
```{r}
cluster_palette <- c("indianred2", "red", "darkred", "darkorchid2", "orange",
...
hm <- hm@matrix
hm <- hm[, c(3,4,5,6,10,15,16,19,21,22,24,26,30)]
...
levels =c("CD4+T", "CD8+T", "T", "NK", "B","classical_mono", "non_classical_mono", "DC", "neutrophil"))
...
### an optional line is to filter the neutrophils out to avoid their overwhelming affect on all other cell proportions
```{r}
cluster_counts <- neut.data %>%
  #filter(!labels %in% "neutrophil") %>%
