...
delta_data <- counts_summary %>%
  ...
  mutate(prop = cluster_count / total_count) %>%  # Proportion per cluster
  group_by(subject, cluster_id) %>%
  mutate(
    baseline_prop = prop[time == "0h"],  # or "-72h" if that's your baseline
...
analyse_cluster <- function(cluster_name) {
  dat_clust <- counts_summary %>% filter(labels == cluster_name)
  
  model <- tryCatch({
    glmer(
      cbind(cluster_count, total_cells - cluster_count) ~ treatment * time_new + (1 | donor),
      data = dat_clust,
      family = binomial
    )
  }, error = function(e) {
...
max_prob <- emm_all %>%
  group_by(cluster_id) %>%
  summarise(max_y = max(prob, na.rm = TRUE)) %>%
...
```
```{r}
ggplot(emm_all, aes(x = time_new, y = prob, colour = treatment, group = treatment)) +
